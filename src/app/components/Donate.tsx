...
export const Donate: FC = () => {
  const [donationMode, setDonationMode] = useState<'once' | 'monthly' | 'zelle'>('monthly'); // Includes 'zelle'
  ...
  const handleDonate = () => {
    if (donationMode !== 'zelle' && selectedAmount) {
      setIsModalOpen(true); // Open the modal for Stripe payment (once or monthly)
    } else {
  ...
  return (
    <>
      <div className="py-8 px-4 max-w-lg mx-auto">
        ...
        <div className="flex bg-white rounded-lg justify-center space-x-2 mb-6 w-full">
          ...
          <button
            ...
            className={`py-1.5 font-medium cursor-pointer md:px-6 rounded-lg text-sm w-full md:text-base ${donationMode === 'monthly'
              ? 'bg-[#F2C94C] text-black'
              : 'bg-white text-black'
              }`}
          >
          ...
          <button
            onClick={() => setDonationMode('zelle')}
            className={`py-1.5 font-medium cursor-pointer md:px-6 rounded-lg text-sm w-full md:text-base ${donationMode === 'zelle'
              ? 'bg-[#F2C94C] text-black'
              : 'bg-white text-black'
              }`}
          >
        ...
        {/* Amount Selection (hidden for Zelle) */}
        <h2 className="text-center text-sm md:text-base font-medium p-4 rounded-tr-lg rounded-tl-lg bg-[#F5F5DC]">
          {donationMode === 'zelle' ? 'Zelle Donation Details' : 'Choose an amount to give'}
        </h2>
          ...
          {donationMode !== 'zelle' && (
            <div className="flex justify-center mb-4">
              <button
                ...
                className="w-full bg-[#F2C94C] cursor-pointer text-black py-3 px-6 rounded-sm hover:scale-105 transition-all duration-200 text-sm md:text-base"
              >
                {donationMode === 'monthly' ? 'Give Monthly' : 'Give'}
              </button>
            </div>
          )}
      ...
      {/* Checkout Modal */}
      {donationMode !== 'zelle' && selectedAmount !== null && (
        <CheckoutModal
